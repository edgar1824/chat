...
export class AuthService {
  static register = async (body) => {
    ...
    if (user) throw createError(401, "email or username is already taken");
    if (body.password) {
  ...
  static registerByGoogle = async (body) => {
    let user;
  ...
  static login = async (body) => {
    ...
    if (!isPasswordCorrect) throw createError(400, "Password is incorrect");
  ...
  static refresh = async (refresh_token) => {
    const token_data = TokenService.validateRefreshToken(refresh_token);
    if (!token_data) throw createError(401, "Not Authenticated");
    const userData = await Token.findOne({ userId: token_data._id });
    if (!userData) throw createError(401, "Not Authenticated");
