...
_a = ConversationService;
ConversationService.delete = async (id, me) => {
    const conv = await Conversation.findOne({ _id: id });
    if (conv.type === "group" &&
        !((conv?.admins?.length &&
            conv.admins.includes(me?._id)) ||
            me?.isAdmin) &&
    ...
    if (messages) {
        await Promise.all(messages.map((m) => MessageService.delete({ _id: m?._id })));
    }
    UploadService.delete(conv?.img);
    await conv.deleteOne();
    return conv;
};
ConversationService.get = async (...options) => {
    const conv = await Conversation.findOne(...options);
    return conv;
};
ConversationService.edit = async ({ id, user, values, }) => {
    const conv = await Conversation.findOne({ _id: id });
    if (values?.img && values.img !== conv?.img)
        UploadService.delete(conv.img);
    const newConv = await Conversation.findOneAndUpdate({ _id: id }, { $set: { ...values } }, { new: true });
    return newConv;
};
ConversationService.create = async ({ members, img, title, _id, ...rest }) => {
...
ConversationService.getOfUser = async (userId) => {
...
ConversationService.getUsersOfConversation = async (conversationId) => {
    const conv = await _a.get({ _id: conversationId });
    const users = await Promise.all(conv.members.map((member) => UserService.getPrivate(member)));
    return users;
};
ConversationService.check = async (members, type) => {
    const cand = await Conversation.findOne({ members: [...members], type });
    return cand;
};
ConversationService.addUser = async (userId, conversationId, user) => {
    ...
    if (!((conv?.admins?.length && conv.admins.includes(user._id)) ||
        user.isAdmin)) {
        throw createError(500, "you are not Admin in this group");
    }
...
ConversationService.deleteUser = async (userId, conversationId, user) => {
    const conv = await Conversation.findOne({ _id: conversationId });
    if (conv.type === "dialogue") {
        if (userId === user?._id) {
            const leftUser = await UserService.get(userId);
            const res = await _a.delete(conversationId, user);
            return {
    ...
    if (!res?.members?.length) {
        await _a.delete(conversationId, user);
        return {
...
ConversationService.makeAdmin = async (conversationId, userId) => {
    const conv = await Conversation.findOne({ _id: conversationId });
    if (conv.type === "dialogue")
...
ConversationService.unmakeAdmin = async (conversationId, userId) => {
    const conv = await Conversation.findOne({ _id: conversationId });
    if (conv.type === "dialogue")
        throw createError(403, "There is no Admins in dialogue!");
    if (!conv?.admins?.length || !conv.admins.includes(userId)) {
