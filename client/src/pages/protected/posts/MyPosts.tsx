...
const Component = () => {
  ...
  return (
    <div className="flex-[1] grid grid-cols-[repeat(auto-fill,_minmax(280px,_1fr))] gap-5 md:grid-cols-2 md:gap-2 sm:!grid-cols-1">
      {!!data?.myPosts?.length ? (
        data?.myPosts?.map((p) => <Post key={p._id} {...p} />)
      ) : (
